...
; This is the non-caching bios read/write routines.
...
.bios_write_miss:
	...
	; pad the SD buffer with all 0xe5
...
.bios_write_prerd:
	...
	; XXX This is a hack that won't work unless the top of the disk partition < 0x10000
	; XXX This has the SD card partition offset hardcoded in it!!!
	...
	or	    a			            ; was the SD driver read OK?
	jp	    z, .bios_write_sdbuf

	call	iputs
	asciiz  "BIOS_WRITE SD CARD PRE-READ FAILED!\r\n"
	ld	    a, 1			        ; tell CP/M the read failed
...
.bios_write_sdbuf:
	...
	or	    a                       ; tell CP/M read succeeded
	jp	    z, .bios_write_ret

	call	iputs
	asciiz	"BIOS_WRITE SD CARD WRITE FAILED!\r\n"
	ld	    a, 1			        ; tell CP/M the read failed
	ld	    (bios_sdbuf_val), a	    ; mark the .bios_sdbuf as invalid
